...
CS.onDrop = function(model) {
    CS.droppables.forEach(function(droppable) {
        ...
        } else if (CS.positionDelta({
                x: ((model.location.x + model.size.width) + model.location.x) / 2,
                y: (model.location.y + (model.size.height + model.location.y)) / 2
            }, droppable.location, droppable.size.width, droppable.size.height)) {
...
define(function(require) {
    ...
    function CellStructureScreen() {
        ...
        Screen.call(this, cellStructureSimString, icon,
            ...
            function(model) {
                return new CellStructureScreenView(model);
            }, {
